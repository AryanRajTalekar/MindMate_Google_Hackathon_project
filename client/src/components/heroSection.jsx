      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/40 px-4 text-center">
        {showWelcome && (
          <motion.h1
            ...
            animate={{ opacity: 1, y: moveUp ? -100 : 0 }}
            transition={{ duration: 1.2, ease: "easeInOut" }}
            className="text-white text-4xl md:text-6xl font-bold"
          >
            Welcome, {name} To Your{" "}
            Safe Space
          </motion.h1>
        )}

        {/* Absolutely positioned typewriter to avoid layout shift */}
        {showFeatures && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 1 }}
            className="absolute bottom-[25%] text-white text-3xl md:text-4xl flex items-center justify-center mb-8"
          >
            <Typewriter
              options={{
                strings: features,
